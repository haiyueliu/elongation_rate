...
### Requirements
  1. Install the required packages.
     ...
     ggplot2
     cowplot
  2. Download the help_functions.R in the R folder in this repository and source it.

### Step 1. Filter gene annotation 
Filter overlapping genes and genes above certain gene length ranges

gtf.file <- "/path/to/gtf" ### full path to annotation file in gtf format (Gencode / Ensembl)
min.genewidth <- 30000
...
### Step 2. Count read depth for each gene at single base resolution
...
write.table(rate_df, paste0(output.dir, "elongation_rate_wave_front.txt"), quote=FALSE, sep="\t", row.names = FALSE)
